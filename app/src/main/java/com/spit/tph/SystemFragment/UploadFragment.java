...
import com.spit.tph.Entity.SPEntityP2.BorrowListRequest;
import com.spit.tph.Entity.SPEntityP2.BriefAsset;
import com.spit.tph.Entity.SPEntityP2.BriefBorrowedList;
import com.spit.tph.Entity.SPEntityP2.PendingReturnAsset;
import com.spit.tph.Entity.SPEntityP2.PhotoUploadRequest;
import com.spit.tph.Entity.SpEntity.StockTakeDetail;
import com.spit.tph.Entity.SpEntity.StrJson;
import com.spit.tph.Entity.SpEntity.UploadStockTakeData;
import com.spit.tph.Entity.Status;
import com.spit.tph.Entity.StockTakeList;
import com.spit.tph.Entity.User;
...
import com.spit.tph.Event.LoginDownloadProgressEvent;
import com.spit.tph.Event.ModifyAssetRequest;
...
import com.spit.tph.WebService.Callback.GetBorrowListAssetCallback;
import com.spit.tph.WebService.Callback.GetBriefAssetCallback;
import com.spit.tph.WebService.Callback.GetBriefBorrowedAssetCallback;
import com.spit.tph.WebService.Callback.GetSPAssetListCallback;
import com.spit.tph.WebService.Callback.GetStockTakeListCallback;
import com.spit.tph.WebService.Callback.ImageReturnCallback;
import com.spit.tph.WebService.Callback.NewAssetDetailCallback;
import com.spit.tph.WebService.Callback.SPWebServiceCallback;
import com.spit.tph.WebService.Callback.UpdateAssetEpcCallback;
...
import static com.spit.tph.SystemFragment.DownloadFragment.BORROW_API_2;
import static com.spit.tph.SystemFragment.DownloadFragment.STOCK_TAKE_API;
...
public class UploadFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater li, ViewGroup vg, Bundle b) {
        ...
        view.findViewById(R.id.tv_wheelchair).setOnClickListener(v -> {
            onWheelchairUpload();
        });
...
,*/
    ...
    private void onWheelchairUpload() {
        if (((MainActivity) MainActivity.mContext).isNetworkAvailable()) {
            Realm.getDefaultInstance().executeTransactionAsync(realm -> {
                try {
                    JSONArray array = new JSONArray();
                    RealmResults<WheelchairBean> wheelchairBeans = realm.where(WheelchairBean.class)
                            .equalTo("companyid", companyId)
                            .equalTo("userId", serverId)
                            .equalTo("uploadStatus", 1)
                            .findAll();
                    if (wheelchairBeans.size() != 0) {
                        for (WheelchairBean bean : wheelchairBeans) {
                            WheelchairItemsBean wheelchairItemsBean = realm.where(WheelchairItemsBean.class)
                                    .equalTo("companyid", companyId)
                                    .equalTo("userId", serverId)
                                    .equalTo("arono", bean.getRono())
                                    .findFirst();
                            JSONObject object = new JSONObject();
                            object.put("arono", bean.getRono());
                            object.put("checkDate", bean.getCheckDate());
                            JSONArray itemJsonArray = new JSONArray();

                            Type arrayType = new TypeToken<List<WheelchairItemsBean>>() {
                            }.getType();
                            List<WheelchairItemsBean> wheelchairItemsList = new Gson().fromJson(wheelchairItemsBean.getList(), arrayType);
                            for (WheelchairItemsBean wheelchairItemsBean1 : wheelchairItemsList) {
                                JSONObject obj = new JSONObject();
                                obj.put("CheckDate", wheelchairItemsBean1.getCheckDate());
                                obj.put("Status", wheelchairItemsBean1.getItemStatus());
                                obj.put("Remarks", wheelchairItemsBean1.getRemarks());
                                obj.put("CheckID", wheelchairItemsBean1.getId());
                                obj.put("FinDate", "");
                                obj.put("SubRoNo", "");
                                itemJsonArray.put(obj);
                            }

                            object.put("itemStatus", itemJsonArray);
                            array.put(object);
                        }
                        JSONObject wheelchairObj = new JSONObject();
                        wheelchairObj.put("wheelchair", array);
                        Log.e("xxxxxxxxxxxxxxxxxxxx", "" + companyId + userid + "-" + wheelchairObj);

                        RetrofitClient.getSPGetWebService().multipleWheelchairUpload(companyId, userid, wheelchairObj.toString())
                                .enqueue(new Callback<WheelchairUploadBean>() {
                                    @Override
                                    public void onResponse(Call<WheelchairUploadBean> call, Response<WheelchairUploadBean> response) {
//                                    if (response.body().getCode() == 200){
                                        Realm.getDefaultInstance().executeTransactionAsync(realm -> {
                                            ...
                                            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                                                RealmResults<WheelchairItemsBean> wheelchairItemsBeans = null;
                                                wheelchairItemsBeans = realm.where(WheelchairItemsBean.class)
                                                        .equalTo("companyid", companyId)
                                                        .equalTo("userId", serverId)
                                                        .in("arono", wheelchairBeans.stream().map(WheelchairBean::getRono).toArray(String[]::new))
                                                        .findAll();
                                                for (WheelchairItemsBean wheelchairItemsBean : wheelchairItemsBeans) {
                                                    wheelchairItemsBean.setUploadStatus(0);
                                                }
                                                realm.insertOrUpdate(wheelchairBeans);
                                                realm.insertOrUpdate(wheelchairItemsBeans);
                                            }
                                        }, () -> {
                                            hideUIIfNeeded();
                                        });
//                                    }
                                    ...
                                    @Override
                                    public void onFailure(Call<WheelchairUploadBean> call, Throwable t) {
...
/*
    ...
    public void hideUIIfNeeded() {
        RealmResults<WheelchairBean> wheelchairList = Realm.getDefaultInstance().where(WheelchairBean.class)
                ...
                .equalTo("uploadStatus", 1)
                .findAll();
        if (wheelchairList.size() > 0) {
            view.findViewById(R.id.ly_wheelchair).setVisibility(View.VISIBLE);
        } else {
            view.findViewById(R.id.ly_wheelchair).setVisibility(View.GONE);
        }
...
/*
        ...
        if (view.findViewById(R.id.ly_wheelchair).getVisibility() == View.GONE &&
                view.findViewById(R.id.change_epc).getVisibility() == View.GONE && view.findViewById(R.id.stock_take_list).getVisibility() == (View.GONE) && view.findViewById(R.id.registration).getVisibility() == (View.GONE) && view.findViewById(R.id.borrow_list).getVisibility() == (View.GONE) && view.findViewById(R.id.disposal_list_wrapper).getVisibility() == (View.GONE) && view.findViewById(R.id.return_list_wrapper).getVisibility() == (View.GONE)) {
